...
motionSeries = [((floor(mod((1:160) - 1, 16)/8)*-2+1)*120)' ; ((floor(mod((1:1520) - 1, 4)/2)*-2+1)*480)'; ((floor(mod((1:160) - 1, 16)/8)*-2+1)*120)' ;0]; %compression cell motion series (in steps) 800 steps = 1/20 in
%motionSeriesTemp = CompressionPercent*[((floor(mod((1:160) - 1, 16)/8)*-2+1)*120)' ; ((floor(mod((1:1520) - 1, 4)/2)*-2+1)*480)'; ((floor(mod((1:160) - 1, 16)/8)*-2+1)*120)' ;0]; %compression cell motion series (in steps) 800 steps = 1/20 in
%motionSeries = motionSeriesTemp(351:1841);
%motionSeries = 0;
numberOfScans = length(motionSeries);

%Input Save Settings
target_folder = 'E:\Run_081017\';
scanFolderName = 'Scan';
ImgName = 'Image';
Extension = '.tif';
...
if(err == 0)
    for n = 70:length(image_stack(1,1,:)) %Don't save bad bottom images
        ima = image_stack(:,:,n)'; %contrast adjust and invert the image, turned off
        ima = ima(620:1490, 930:1830); %crop image before saving (y1:y2,x1:x2)
