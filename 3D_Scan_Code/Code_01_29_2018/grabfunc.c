...
EXPORTED_FUNCTION void pco_edge_transferpar(void *hcam)
{
   ...
   err=PCO_GetCameraType(hcam,&camtype);
   if(err!=0)
    mexPrintf("\nPCO_GetCameraType returned error 0x%x\n",err);
   ...
   if(camtype.wCamType==CAMERATYPE_PCO_EDGE)
   {
   ...
   err=PCO_CamLinkSetImageParameters(hcam,wXResAct,wYResAct);
   if(err!=0)
    mexPrintf("\nPCO_CamLinkSetImageParameters returned error 0x%x\n",err);
