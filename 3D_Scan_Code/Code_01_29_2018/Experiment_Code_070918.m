...
% (AP) 07/09/18 This new version of the program, try to account for the
% backlash in the compression wall motion.
...
NumberOfCycles = 600; %In number of cycles, of those the first ten and the last ten would be high resolution ones.
...
motionSeries=motion_series_with_bl(NumberOfCycles,CompressionDistance,BacklashDistance);
numberOfScans = length(motionSeries);

%Input Save Settings and create directory
target_folder = 'X:\ExperimentsRawData\CyclicCompression\Experiment_07_09_18\';
mkdir(target_folder);
...
tic
for scanNumber = 101:numberOfScans
