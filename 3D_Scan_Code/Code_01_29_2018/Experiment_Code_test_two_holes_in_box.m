...
scan_distance = 20; %in mm, the height of laser scanning
volume_length=6.115; %in inches, the length of the current compression box
...
NumberOfCycles = 800; %In number of cycles, of those the first ten and the last ten would be high resolution ones.


%Input Parameters for compression cell motion
CompressionSpeed = 0.05; % In mm per second
CompressionPercent=3.0; %Relative to the "current" size of container
...
target_folder = 'X:\ExperimentsRawData\CyclicCompression\Test_two_hole_box_050818_try6\';
mkdir(target_folder);
