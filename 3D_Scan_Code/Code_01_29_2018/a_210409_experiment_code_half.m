...
scan_distance = 60; %in mm, the height of laser scanning
...
% put in mm
% -------------------------------------------------------------------------
CompressionDistance = 2.50;
% -------------------------------------------------------------------------
% full amplitude in steps
% -------------------------------------------------------------------------
CompressionSteps    = floor(CompressionDistance / in2mm * Inch2Step);
% I am dividing it by 8 -> good
...
motionSeries = -floor(CompressionSteps/8) * (2*mod(floor(linspace(0,15,16)/8),2)-1)';
numberOfScans = length(motionSeries);

% -------------------------------------------------------------------------
% -------------------------------------------------------------------------
%Input Save Settings and create directory
target_folder = 'X:\ExperimentsRawData\CyclicCompressionTwoHoles\Experiment_210409\';
mkdir(target_folder);
...
% First 10 scans
for cycleNum = 1:10
    disp(['Cycle: ',num2str(cycleNum)]);
    cntr =  16*cycleNum + 1;
    % Start 16 scans per whatever
    if (cycleNum == 10)
        nScans = 17;
    else
        nScans = 16;
    end
    for scanNumber = 1:nScans
        ...
        if(err == 0)
            ...
            create_hdf5(cntr,imacount,XResolution,YResolution,target_folder);
            save_to_hdf5(image_stack(LoLimX:UpLimX,LoLimY:UpLimY,:),cntr,target_folder)
...
% The last 10 cycles that I record
for cycleNum = 401:410
    disp(['Cycle: ',num2str(cycleNum)]);
    cntr =  16*cycleNum + 1;
    ...
    for scanNumber = 1:nScans
        ...
        if(scanNumber <= 16)
            disp(['Wall Counter: ',num2str(scanNumber)]);
            disp(['Moving wall: ',num2str(motionSeries(scanNumber))]);
            % moves it based on the scan number
            moveWall(motionSeries(scanNumber),s4);
        end
        %take note of the time motor motion started
        motionStartTime = cputime;
