...
pixel_width = volume_length*25.4/816; %in mm, the correspondance between pixel size and real distance
...
CompressionPercent=2; %Relative to the "current" size of container
...
%Input Save Settings
target_folder = 'X:\ExperimentsRawData\CyclicCompression\Experiment_01_29_18\';
scanFolderName = 'Scan';
ImgName = 'Image';
Extension = '.tif';
...
for scanNumber = 1:numberOfScans
    ...
    if(err == 0)
        ...
        %Save first image
        save_image=image_stack(LoLimX:UpLimX,LoLimY:UpLimY,1)';
        filename = [target_folder, currentScanName, ImgName, num2str(1, '%03.0f'), Extension];
        imwrite(save_image,filename,'Compression','deflate'); %save image to folder
        
        %Append all other images to the same file
        for n = 2:imacount
            save_image=image_stack(LoLimX:UpLimX,LoLimY:UpLimY,n)';
            imwrite(save_image,filename,'Compression','deflate','WriteMode','append'); %save image to folder
        end
    end
