...
motionSeries = [((floor(mod((1:160) - 1, 16)/8)*-2+1)*120)' ; ((floor(mod((1:1400) - 1, 4)/2)*-2+1)*480)'; ((floor(mod((1:160) - 1, 16)/8)*-2+1)*120)' ;0]; %compression cell motion series (in steps) 800 steps = 1/20 in
numberOfScans = length(motionSeries);

%Input Save Settings
target_folder = 'F:\DataRun_071015\';
scanFolderName = 'Scan';
ImgName = 'Image';
Extension = '.tif';
...
if(err == 0)
    for n = 1:length(image_stack(1,1,:))
        ...
        ima = ima(220:1328,110:1230); %crop image before saving
