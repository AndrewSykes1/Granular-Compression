...
motionSeriesTemp = [((floor(mod((1:160) - 1, 16)/8)*-2+1)*120)' ; ((floor(mod((1:1520) - 1, 4)/2)*-2+1)*480)'; ((floor(mod((1:160) - 1, 16)/8)*-2+1)*120)' ;0]; %compression cell motion series (in steps) 800 steps = 1/20 in
motionSeries = motionSeriesTemp(1101:1841);
%motionSeries = 0;
numberOfScans = length(motionSeries);

%Input Save Settings
target_folder = 'F:\DataRun_111115\Pt2\';
scanFolderName = 'Scan';
ImgName = 'Image';
Extension = '.tif';
...
foldrnum=scanNumber+1100;
currentScanName = [scanFolderName, num2str(foldrnum, '%03.0f'), '/'];
...
if(err == 0)
    for n = 1:length(image_stack(1,1,:))
        ...
        ima = ima(95:595,85:585); %crop image before saving (y1:y2,x1:x2)
