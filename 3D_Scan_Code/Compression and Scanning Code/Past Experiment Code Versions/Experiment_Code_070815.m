...
scan_distance = 68; %in mm
pixel_width = 148/872; %in mm
...
CompressionSpeed = 100; % speed in steps per second (max 100)
motionSeries = ((floor(mod((1:200) - 1, 4)/2)*-2+1)*492)'; %compression cell motion series (in steps) 800 steps = 1/20 in
numberOfScans = length(motionSeries);

%Input Save Settings
target_folder = 'F:\1% compression 1 cycle 4 frames\';
scanFolderName = 'Scan';
ImgName = 'Image';
Extension = '.tif';
...
if(err == 0)
    for n = 31:length(image_stack(1,1,:))
        ima = image_stack(:,1:end,n)'; %contrast adjust and invert the image, turned off
        ima = ima(150:1290,200:1328); %crop image before saving
